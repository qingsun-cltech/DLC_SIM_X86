...
inline double8_128 ll2double(longlong8_128 x){
    ...
    // printf("const_temp_reg0 = %x\n", const_temp_reg0[0]);
    // printf("const_temp_reg1 = %x\n", const_temp_reg1[0]);
    // 再对x-1按位取反
    ...
    // printf("int64_lo_reg = %x\n", int64_lo_reg[0]);
    // printf("int64_hi_reg = %x\n", int64_hi_reg[0]);
    // clz 为了找到第一个1
    clz_temp_hi_reg = v_u32_clz(int64_hi_reg);
    clz_temp_lo_reg = v_u32_clz(int64_lo_reg);
    // printf("clz_temp_hi_reg = %x\n", clz_temp_hi_reg[0]);
    // printf("clz_temp_lo_reg = %x\n", clz_temp_lo_reg[0]);
    ...
    // printf("frac_temp_hi_reg = %x\n", frac_temp_hi_reg[0]);
    // printf("frac_hi_reg = %x\n", frac_hi_reg[0]);
    // printf("double_lo_reg = %x\n", double_lo_reg[0]);
    // printf("double_hi_reg = %x\n", double_hi_reg[0]);
    double8_128 res;
...
inline double8_128 reduce_large(int8_128 x, int8_128 *n)
{
    ...
    *n = v_u32_shr(v_s32_add(v_u32_shr(res0.hi, 16), v_u32_move_i(0x2000)) & 0xffff, 14);
    ...
    // printf("pi63.hi = %x\n", pi63.hi[0]);
    // printf("pi63.lo = %x\n", pi63.lo[0]);
    // printf("ll2double(res0).hi = %x\n", ll2double(res0).hi[0]);
    // printf("ll2double(res0).lo = %x\n", ll2double(res0).lo[0]);
...
inline int8_128 rem_pio2f(float8_128 x, int8_128 *y0, int8_128 *y1)
{
    ...
    // printf("mask = %x\n", v_s32_sel(mask, 0, 1)[0]);
    double8_128 double_y0;
    // printf("reduce_large_dx.hi = %x\n", reduce_large_dx.hi[0]);
    // printf("reduce_large_dx.lo = %x\n", reduce_large_dx.lo[0]);
    // printf("reduce_fast_dx.hi = %x\n", reduce_fast_dx.hi[0]);
    // printf("reduce_fast_dx.lo = %x\n", reduce_fast_dx.lo[0]);
    ...
    *y0 = double2float(double_y0);
    // printf("double_y0.hi = %x\n", double_y0.hi[0]);
    // printf("double_y0.lo = %x\n", double_y0.lo[0]);
    // printf("y0 = %x\n", (*y0)[0]);
...
inline float8_128 __kernel_cosf(float8_128 x, float8_128 y)
{
    ...
    // if (get_device_id()) printf("z = %x\n", z[0]);
    // if (get_device_id()) printf("r = %x\n", r[0]);
...
{
    // return process1024(x, [](float x) { return std::cos(x);});
    float8_128 res;
    int8_128 ix = _$S(x);
    // if (get_device_id()) Print("ix = %h\n", ix);
    ix = ix & 0x7fffffff;

    float8_128 z = v_u32_move_f(0);
    // if (get_device_id()) Print("z = %h\n", z);
    
    int8_128 y0, y1;
    int8_128 n = rem_pio2f(x, &y0, &y1);
    // if (get_device_id()) Print("n = ", n, PrintType::HEX);
    // if (get_device_id()) Print("y0 = ", y0, PrintType::HEX);
    // printf("y0 = %x\n", y0[0]);
    n = n & 3;
    // if (get_device_id()) Print("n = %h\n", n);

    float8_128 kernel_cos_res = __kernel_cosf(_$F(y0), _$F(y1));
    float8_128 kernel_sin_res = __kernel_sinf(_$F(y0), _$F(y1), 1);
    // if (get_device_id()) printf("kernel_cos_res = %x\n", kernel_cos_res[0]);
    // if (get_device_id()) Print("kernel_cos_res = ", kernel_cos_res, PrintType::HEX);
    // if (get_device_id()) Print("kernel_sin_res = ", kernel_sin_res, PrintType::HEX);

    res = kernel_sin_res;
    bool8_128 mask0 = v_s32_cmp(EQ, n, v_u32_move_i(0));
    // if (get_device_id()) Print("mask0 = %h\n", v_s32_sel(mask0, 0, 1));
    res = v_f32_sel(mask0, res, kernel_cos_res);
    bool8_128 mask1 = v_s32_cmp(EQ, n, v_u32_move_i(1));
    // if (get_device_id()) Print("mask1 = %h\n", v_s32_sel(mask1, 0, 1));
    res = v_f32_sel(mask1, res, v_f32_mul_b(kernel_sin_res, v_u32_move_f(-1)));
    bool8_128 mask2 = v_s32_cmp(EQ, n, v_u32_move_i(2));
    // if (get_device_id()) Print("mask2 = %h\n", v_s32_sel(mask2, 0, 1));
    ...
    // if (get_device_id()) Print("mask = %h\n", v_s32_sel(mask, 0, 1));
    res = v_f32_sel(mask, res, __kernel_cosf(x, z));

    bool8_128 infnan = v_f32_infnan(x);
    // if (get_device_id()) Print("infnan = %h\n", v_s32_sel(infnan, 0, 1));
