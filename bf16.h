...
inline int8_128 __$S(const float8_128& x) {
  int8_128 y;
  dlc_dtype val;
...
inline float8_128 __$F(const int8_128& x) {
  float8_128 y;
  dlc_dtype val;

  for (int i = 0; i < 1024; ++i) {
    val.u32 = (uint32_t)x[i];
    y[i] = val.f32;
    // int x_i = x[i];
    // y[i] = *reinterpret_cast<float*>(&x_i);
  }
