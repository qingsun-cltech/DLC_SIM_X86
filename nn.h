...
inline void matmul_LHS_aw256_pipeline_f32(SIM_X86::tensor A, SIM_X86::tensor C, int ah, int bw, int iaw, int ibw, int add_src_flag) {
    ...
    int iah = 0;
    if (get_device_id()) {
        // printf("[XSY%d]: iaw = %d, add_src_flag = %d\n", get_device_id(), iaw, add_src_flag);
    }
    for (; iah < ah256; iah += 256) {
    // for (; iah < 512; iah += 256) {
        matmul_2pgx_256_pre16(A + ah * iaw / 32 + iah * 4, A + ah * (iaw + 128) / 32 + iah * 4);
        if (get_device_id()) {
            // Print("matmul_2pgx_256_pre16 = %f\n", ( A + ah * (iaw + 128) / 32 + iah * 4), 128, PrintType::FLOAT);
        }
        ...
        if (get_device_id()) {
            // Print("out = %f\n", (A + ah * (iaw + 128) / 32 + 4 * 128 + iah * 4), 128, PrintType::FLOAT);
        }
    }
