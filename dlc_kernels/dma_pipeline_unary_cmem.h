...
// #pragma clang loop unroll_count(16)
    for (int i = 0; i < len1024; i += 1024) {
        ...
        v_f32_st_tnsr_b(i / 32, vmem, fn(val, aux));
    }
    if (len != len1024) {
        int ldmk = (1 << ((len % 1024) / 128)) - 1;
        float8_128 val = v_f32_ld_tnsr_st_msk(len1024 / 32, vmem, 1, ldmk);
        v_f32_st_tnsr_st_msk(len1024 / 32, vmem, 1, ldmk, fn(val, aux));
    }
}
...
inline void element_wise_lanhu_model(SIM_X86::tensor hbmin, SIM_X86::tensor hbmout, int numel, SIM_X86::tensor vmem, int vmemlen,
    ...
    // we pipeline the dma
    if (firstForLoop) {
        flagIn0 = dlc_dma(hbmin + l / 32, CMEM, vmem, VMEM, half_vmem, 128, 128, 128, 7);
    }
    for (; l + vmemlen <= numel; l += vmemlen) {
        ...
        dlc_sync(flagOut1);
        flagIn1 = dlc_dma(hbmin + (l + half_vmem) / 32, CMEM, vmem + half_vmem / 32, VMEM, half_vmem, 128, 128,
                          128, 7);
        dlc_sync(flagIn0);
        ...
        flagOut0 = dlc_dma(vmem, VMEM, hbmout + l / 32, CMEM, half_vmem, 128, 128, 128, 7);
        dlc_sync(flagOut0);

        flagIn0 = dlc_dma(hbmin + (l + 2 * half_vmem) / 32, CMEM, vmem, VMEM, half_vmem, 128, 128, 128, 7);
        dlc_sync(flagIn1);
        ...
        flagOut1 = dlc_dma(vmem + half_vmem / 32, VMEM, hbmout + (l + half_vmem) / 32, CMEM, half_vmem, 128,
                           128, 128, 7);
    }
    if (firstForLoop) {
        dlc_sync(flagOut1);
    ...
    if (l < numel) {
        int curvmem = numel - l;
        int flagA = dlc_dma(hbmin + l / 32, CMEM, vmem, VMEM, curvmem, 128, 128, 128, 7);
        dlc_sync(flagA);
        element_wise_transform_inplace(vmem, curvmem, fn, aux);
        int flagB = dlc_dma(vmem, VMEM, hbmout + l / 32, CMEM, curvmem, 128, 128, 128, 7);
        dlc_sync(flagB);
    }
}

inline void manual_dma_h2v_flag1(SIM_X86::tensor src, SIM_X86::tensor dst, int len) {
    asm volatile("{ pseudo@0 @pseudo imm_5 = 1; "
                 "  pseudo@0 @pseudo imm_1 = 7; "
                 "  S0@(pr0) r2 = shr.u32 %[len], r33; "
                 "  MISC@(pr0) 7 = setsync.clear 0; }"
                 "{ pseudo@0 @pseudo imm_0 = 2; "
                 "  S1@(pr0) r0 = shr.u32 %[src], r32; }"
                 "{ pseudo@0 @pseudo imm_0 = 2; "
                 "  S1@(pr0) r1 = shr.u32 %[dst], r32; }"
                 "{ pseudo@0 @pseudo imm_2 = 16385; "
                 "  pseudo@0 @pseudo imm_3 = 16384; "
                 "  S0@(pr0) [dest:r1], [sflag:r45] = dmalocal [src:r0], r2, _4384; } "
                 :
...
inline void manual_dma_h2v_flag2(SIM_X86::tensor src, SIM_X86::tensor dst, int len) {
    asm volatile("{ pseudo@0 @pseudo imm_5 = 2; "
                 "  pseudo@0 @pseudo imm_1 = 7; "
                 "  S0@(pr0) r2 = shr.u32 %[len], r33; "
                 "  MISC@(pr0) 7 = setsync.clear 0; }"
                 "{ pseudo@0 @pseudo imm_0 = 2; "
...
inline void manual_dma_v2h_flag1(SIM_X86::tensor src, SIM_X86::tensor dst, int len) {
    asm volatile("{ pseudo@0 @pseudo imm_5 = 1; "
                 ...
                 "{ pseudo@0 @pseudo imm_2 = 16385; "
                 "  pseudo@0 @pseudo imm_3 = 16384; "
                 "  S0@(pr0) [dest:r1], [sflag:r45] = dmalocal [src:r0], r2, _2688; } "
                 :
                 : [src] "r"(src), [dst] "r"(dst), [len] "r"(len)
                 : "r0", "r1", "r2", "r3");
}
...
inline void manual_dma_done_flag1() {
    asm volatile("{ pseudo@0 @pseudo imm_5 = 1; MISC@(pr0) 7 = setsync.done 0; }" : : :);
}

inline void manual_dma_done_flag2() {
    asm volatile("{ pseudo@0 @pseudo imm_5 = 2; MISC@(pr0) 7 = setsync.done 0; }" : : :);
}

inline void manual_dma_sync_flag1() {
    asm volatile("{ pseudo@0 @pseudo imm_5 = 1; MISC@(pr0) Nah = wait.done 7, 0; }"
                 "{ S0@(pr0) Nah = fence; } "
                 :
                 :
                 :);
...
inline void manual_dma_sync_flag2() {
    asm volatile("{ pseudo@0 @pseudo imm_5 = 2; MISC@(pr0) Nah = wait.done 7, 0; }"
                 "{ S0@(pr0) Nah = fence; } "
                 :
                 :
                 :);
...
inline void element_wise_qingshan_model(SIM_X86::tensor in_hbm, SIM_X86::tensor out_hbm, int numel, SIM_X86::tensor vmem, int vmemlen,
    ...
    SIM_X86::tensor B = vmem + half_vmem / 2;
    int hbm_len = numel;
