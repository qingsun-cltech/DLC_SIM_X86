...
inline void bf16ToF32(SIM_X86::tensor mem, int len, int d0) {
    ...
    if (d0k128 % 2 == 0) {
        ...
        int h = len / (bd0 / 2);
        for (int i = h - 1; i >= 0; i--) {
            __bf16ToF32_128(mem + i * bd0 / 2 / 32, mem + i * d0 / 32, bd0 / 2);
        }
    }
}
...
inline void __f32ToBf16_256_cmem(SIM_X86::tensor input, SIM_X86::tensor output, int _len, bool is_128) {
    int len = _len * 2;
    for (int i = 0, j = 0; i < len; i += 2048, j += 1024) {
        int l = min(_len - j, 1024);
        int ldst_mask = pre_exp2(l / 128);
        ...
        float8_128 x1 = v_f32_fxc_load(i / 32,input , 2, ldst_mask);
        float8_128 x2 = v_f32_fxc_load((i + 128) / 32, input , 2, ldst_mask2);
...
inline void f32ToBf16(SIM_X86::tensor mem, int len, int d0) {
    ...
    if (d0k128 % 2 == 0) {
        ...
        int h = len / (bd0 / 2);
        for (int i = 0; i < h; i++) {
            __f32ToBf16_256(mem + i * d0 / 32, mem + i * bd0 / 2 / 32, bd0 / 2, true);
        }
    }
}
...
// len: target bf16 mem len (4B)
// d0: before all padding
inline void f32ToBf16_cmem(SIM_X86::tensor mem, int len, int d0) {
    d0 = ((d0 + 127) / 128) * 128;
    int d0k128 = d0 / 128;
    if (d0k128 % 2 == 0) {
        __f32ToBf16_256_cmem(mem, mem, len, false);
    } else {
        int bd0 = ((d0 + 255) / 256) * 256;
        int h = len / (bd0 / 2);
        for (int i = 0; i < h; i++) {
            __f32ToBf16_256_cmem(mem + i * d0 / 32, mem + i * bd0 / 2 / 32, bd0 / 2, true);
        }
    }
}
...
inline void f32ToBf16_v2c(SIM_X86::tensor mem, SIM_X86::tensor cmem_output, int len, int d0) {
    ...
    if (d0k128 % 2 == 0) {
        ...
        int h = len / (bd0 / 2);
        for (int i = 0; i < h; i++) {
            __f32ToBf16_256_v2c(mem + i * d0 / 32, cmem_output + i * bd0 / 2 / 32, bd0 / 2, true);
        }
    }
}

inline void f32ToBf16_v2c_h(SIM_X86::tensor mem, SIM_X86::tensor cmem_output, int len, int h, int d0) {
    d0 = ((d0 + 127) / 128) * 128;
    int d0k128 = d0 / 128;
    if (d0k128 % 2 == 0) {
        __f32ToBf16_256_v2c(mem, cmem_output, len, false);
