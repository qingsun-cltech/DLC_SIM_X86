...
#ifndef _SINCOSF_H_X86_
#define _SINCOSF_H_X86_

#include "cosf.h"

inline float2 __dlc_sincosf(float8_128 x)
{
    float8_128 sin_res, cos_res;
    ...
    float8_128 kernel_cos_res = __kernel_cosf(*(float8_128*)(&y0), *(float8_128*)(&y1));
    float8_128 kernel_sin_res = __kernel_sinf(*(float8_128*)(&y0), *(float8_128*)(&y1), 1);

    sin_res = v_f32_mul_b(kernel_cos_res, v_u32_move_f(-1));
    cos_res = kernel_sin_res;
    bool8_128 mask0 = v_s32_cmp(EQ, n, v_u32_move_i(0));
    sin_res = v_f32_sel(mask0, sin_res, kernel_sin_res);
    cos_res = v_f32_sel(mask0, cos_res, kernel_cos_res);
    bool8_128 mask1 = v_s32_cmp(EQ, n, v_u32_move_i(1));
    sin_res = v_f32_sel(mask1, sin_res, kernel_cos_res);
    cos_res = v_f32_sel(mask1, cos_res, v_f32_mul_b(kernel_sin_res, v_u32_move_f(-1)));
    bool8_128 mask2 = v_s32_cmp(EQ, n, v_u32_move_i(2));
    sin_res = v_f32_sel(mask2, sin_res, v_f32_mul_b(kernel_sin_res, v_u32_move_f(-1)));
    cos_res = v_f32_sel(mask2, cos_res, v_f32_mul_b(kernel_cos_res, v_u32_move_f(-1)));

    // x >= 0x7f800000
    bool8_128 mask = v_s32_cmp(GT, ix, v_u32_move_i(0x7f800000));
    sin_res = v_f32_sel(mask, sin_res, v_f32_sub_b(x, x));
    cos_res = v_f32_sel(mask, cos_res, v_f32_sub_b(x, x));

    // x <= 0x3fe921fb
    mask = v_s32_cmp(LS, ix, v_u32_move_i(0x3fe921fb));
    sin_res = v_f32_sel(mask, sin_res, __kernel_sinf(x, z, 0));
    cos_res = v_f32_sel(mask, cos_res, __kernel_cosf(x, z));

    return (float2){sin_res, cos_res};
}

inline float2 __dlc_sincosf_libdevice(float8_128 a)
{
    float8_128 result0;
    float8_128 result1;
    float2 sin_res;
