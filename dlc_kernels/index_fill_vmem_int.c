...
inline void vmask_fill(SIM_X86::tensor vmem_input, int math_h , int dim0_pad , SIM_X86::tensor smem_index, int index_size ,int value  ){
  ...
  for(; vs + 1024 <= dim0_pad ; vs += 1024){
    ...
    for(int dim0_count = 0; dim0_count < math_h ; dim0_count += 1 ){
      int8_128 value_x = v_i32_ld_tnsr((vs + dim0_count * dim0_pad ) / 32 , vmem_input, 1 ,255);
      int8_128 res = v_s32_sel(cmp_core,value_x,value);
      v_st_generic((vs + dim0_count * dim0_pad ) / 32, vmem_input , 1, 255 , res);
    }
  }
  // 不足1024的处理
  if(vs < dim0_pad){
    ...
    for(int dim0_count = 0; dim0_count < math_h ; dim0_count += 1 ){
      ...
      int8_128 value_x = v_i32_ld_tnsr((vs + dim0_count * dim0_pad ) / 32 , vmem_input, 1 ,ldst_msk);
      int8_128 res = v_s32_sel(cmp_core,value_x,value);
      v_st_generic((vs + dim0_count * dim0_pad ) / 32, vmem_input , 1, ldst_msk , res);
    }
  }
}
...
void main_x86(SIM_X86::DLCMem* mem_info, SIM_X86::DLCTensor* input_hbm0_, 
              ...
              int* _index_size, 
              int* _dim, int* _value) {
// void main(DLCMem* mem_info, DLCTensor* input_hbm0_, DLCTensor* index_, DLCTensor* output_hbm_, DLCScalar* _index_size  ,DLCScalar* _dim,
//            DLCScalar* _value) {
/*##AUTO-GEN##*/TensorFixDims(input_hbm0_);TensorFixDims(index_);TensorFixDims(output_hbm_);/*##END-GEN##*/
  ...
  // void* input_hbm0 = input_hbm0_->address;
  // void* index = index_->address;
  ...
  // int* smem_index = (void*)mem_info->smem_addr;
        ...
        for(int l = 0 ; l < len_xys * 128 ; l += UseVMEMsize){
          ...
          for(; ldst_count + sync_size <= VMEMsize; ldst_count += sync_size){
            ...
            for(int vs = 0; vs < sync_size; vs += 1024 ){
              int8_128 value_x = v_i32_ld_tnsr((vs + ldst_count) / 32 , vmem_input, 1 ,255);
              int8_128 res = v_s32_sel(cmp_core,value_x,value);
              v_st_generic((vs + ldst_count) / 32, vmem_input , 1, 255 , res);
            }
          }
          if(ldst_count < VMEMsize){
            ...
            for(int vs = 0; vs < last_ldst_size; vs += 1024 ){
              ...
              int8_128 value_x = v_i32_ld_tnsr((vs + ldst_count) / 32 , vmem_input, 1 ,ldst_mask);
              int8_128 res = v_s32_sel(cmp_core,value_x,value);
              v_st_generic((vs + ldst_count ) / 32, vmem_input , 1, ldst_mask , res);
            }
          }
...
/*除了最低维度的其他维度*/
      ...
      sync_device();
      int8_128 fill_value = value;
      v_st_generic(0,vmem_input,1,1,fill_value);
//       __attribute__((unused)) volatile float wait = vstore_wait(v_f32_ld_tnsr_st_msk(0,vmem_input,1,1));
      //外部循环的dma的offset
