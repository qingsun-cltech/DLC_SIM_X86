...
inline void store128_128_ex_with_stride2(SIM_X86::tensor t, int h, int w, int ih, int iw, int stride_dst, float128_128 v) {
//     int pw = (w + 127) & 0xffffff80;
//     int cur_w = min(w - iw * 128, 128);
    int cur_h = min(h - ih * 128, 128);
    int kS = (cur_h + 7) / 8;
...
inline void store128_128_ex_with_stride(SIM_X86::tensor dst, int _h, int _w, int h, int output_stride, float128_128 v) {
    int kS = (h + 7) / 8;
#define CASE_ITEM(x)                                                                                         \
    {                                                                                                        \
        int cur_h = min(h - (x) * 8, 8);                                                                     \
...
inline void pop_nws0_store128_128_ex(SIM_X86::tensor t, int h, int w, int ih, int iw) {
    int pw = (w + 127) & 0xffffff80;
//     int cur_w = min(w - iw * 128, 128);
    int cur_h = min(h - ih * 128, 128);
    int kS = (cur_h + 7) / 8;
...
inline void pop_nws1_store128_128_ex(SIM_X86::tensor t, int h, int w, int ih, int iw) {
    int pw = (w + 127) & 0xffffff80;
//     int cur_w = min(w - iw * 128, 128);
    int cur_h = min(h - ih * 128, 128);
    int kS = (cur_h + 7) / 8;
...
inline void tile_trans_transfer_with_stride(SIM_X86::tensor src, SIM_X86::tensor dst, int srcaddr, int src_h, int src_w, int dstaddr, int stride_src, int stride_dst) {
    ...
    for (int j = 0; j < src_w; j += 128) {\
        ...
        store128_128_ex_with_stride2(tensor_slice(dst, dstaddr / 32), src_w, src_h, j / 128, i / 128, stride_dst, v);
    }
}
        ...
        for (idx[4] = idx_origin[4]; idx[4] < oridim[4]; ++idx[4]) {
            ...
            for (idx[3] = idx_origin[3]; idx[3] < oridim[3]; ++idx[3]) {
                ...
                for (idx[2] = idx_origin[2]; idx[2] < oridim[2]; ++idx[2]) {
                    ...
                    int diff = ((int)(input0_hbm + offset_base_src / 32).data_size - (int)(output_hbm + offset_base_dst / 32).data_size);
